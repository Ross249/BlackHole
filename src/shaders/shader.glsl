...
vec2 to_spherical(vec3 cartesian_coord) {
  ...
  vec2 uv =
      vec2(atan(cartesian_coord.z, cartesian_coord.x), asin(cartesian_coord.y));
...
vec3 lorentz_transform_velocity(vec3 u, vec3 v) {
  ...
  if (speed > 0.0) {
    ...
    vec3 new_u =
        (u / gamma - v + (gamma / (gamma + 1.0)) * dot(u, v) * v) / denominator;
    return new_u;
  }
  return u;
}
...
void main() {
  ...
  //  float hfov = (2.0 * ((uv.x+0.5)/resolution.x) - 1.0) * d *
  ...
  // generate ray
  vec3 pixel_pos =
      cam_pos + forward + nright * uv.x * uvfov + up * uv.y * uvfov;
  ...
  for (int i = 0; i < NSTEPS; i++) {
    ...
    bool horizon_mask =
        distance < 1.0 && length(oldpoint) > 1.0; // intersecting eventhorizon
    ...
    if (accretion_disk) {
      if (oldpoint.y * point.y < 0.0) {
        ...
        if (DISK_IN <= r && r <= DISK_IN + DISK_WIDTH) {
          ...
          phi = mod(phi, PI * 2.0);
          float disk_gamma =
              1.0 / sqrt(1.0 - dot(disk_velocity, disk_velocity));
          float disk_doppler_factor =
              disk_gamma *
              (1.0 + dot(ray_dir / distance, disk_velocity)); // from source

          if (use_disk_texture) {
            // texture
            vec2 tex_coord = vec2(mod(phi, 2.0 * PI) / (2.0 * PI),
                                  1.0 - (r - DISK_IN) / (DISK_WIDTH));
            vec4 disk_color = texture2D(disk_texture, tex_coord) /
                              (ray_doppler_factor * disk_doppler_factor);
            float disk_alpha =
                clamp(dot(disk_color, disk_color) / 4.5, 0.0, 1.0);
            ...
            float disk_alpha =
                clamp(dot(disk_color, disk_color) / 3.0, 0.0, 1.0);
  ...
  if (distance > 1.0) {
    ...
    if (star_color.g > 0.0) {
      float star_temperature =
          (MIN_TEMPERATURE + TEMPERATURE_RANGE * star_color.r);
      // arbitrarily sets background stars' velocity for random shifts
      float star_velocity = star_color.b - 0.5;
      float star_doppler_factor =
          sqrt((1.0 + star_velocity) / (1.0 - star_velocity));
      if (doppler_shift)
